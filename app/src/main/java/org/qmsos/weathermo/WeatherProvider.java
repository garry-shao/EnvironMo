...
public class WeatherProvider extends ContentProvider {
    ...
    public int update(@NonNull Uri uri,
        ...
        switch (URI_MATCHER.match(uri)) {
        case CITIES:
            count = database.update(DatabaseHelper.TABLE_CITIES,
                    values, selection, selectionArgs);
            break;
        case WEATHER:
            count = database.update(DatabaseHelper.TABLE_WEATHER,
                    values, selection, selectionArgs);
            break;
        default:
            throw new IllegalArgumentException("Unsupported URI: " + uri);
    ...
    public int bulkInsert(@NonNull Uri uri, @NonNull ContentValues[] values) {
        ...
        case CITIES:
            ...
            try {
                for (ContentValues value : values) {
                    long rowId = database.insert(DatabaseHelper.TABLE_CITIES,
                            "city", value);
                    if (rowId < 0) {
                        return 0;
                    }
        ...
        case WEATHER:
            ...
            try {
                for (ContentValues value : values) {
                    long rowId = database.insert(DatabaseHelper.TABLE_WEATHER,
                            "weather", value);
                    if (rowId < 0) {
                        return 0;
                    }
